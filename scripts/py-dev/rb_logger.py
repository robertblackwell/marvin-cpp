...
class RBLogger(SourcePackage):
	...
	def get_package(self):
		...
		run("git clone file://$\{HOME\}/git-repos/rb_logger", self.defaults.clone_dir)
		super().get_package_after()
	
	def stage_package(self):
		super().stage_package_before()
		run("cp -rv {}/rb_logger/*.hpp {}".format(self.package_clone_dir_path, self.package_stage_external_src_dir_path))
		run("cp -rv {}/rb_logger/*.cpp {}".format(self.package_clone_dir_path, self.package_stage_external_src_dir_path))
