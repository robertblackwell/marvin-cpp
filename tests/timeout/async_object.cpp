...
//        std::cout << "Final count is " << m_count << std::endl;
    ...
    void async_fail_operation(long interval_secs, long to_interval_secs, std::function<void(const boost::system::error_code err)> handler)
    {
        ...
        m_timeout.setTimeout(to_interval, m_strand.wrap([this](){
            m_timer.cancel();
        }));
        auto xhw = m_strand.wrap(std::bind(&AsyncObject::async_fail_op_handler, this, std::placeholders::_1));
        m_timer.async_wait(m_strand.wrap(std::bind(&AsyncObject::async_fail_op_handler, this, std::placeholders::_1)));
    }
    ...
    void async_fail_op_handler(const boost::system::error_code& err)
    {
        ...
        LogDebug(" err: ", s);
        m_timeout.cancelTimeout( m_strand.wrap([this, ec, s](){
            LogDebug(" err: ", s);
            m_op_handler(ec);
        }));
    }
    ...
    void async_operation(long interval_secs, long to_interval_secs, std::function<void(const boost::system::error_code err)> handler)
    {
        ...
        m_timer.async_wait(m_strand.wrap(std::bind(&AsyncObject::async_op_handler, this, std::placeholders::_1)));
    }
