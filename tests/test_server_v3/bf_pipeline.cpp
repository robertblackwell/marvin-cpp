...
void PipelineTest::handler(Marvin::ErrorType& er, MessageReaderSPtr rdr)
{
    ...
    std::string connection_hdr = rdr->getHeader(Marvin::Http::HeadersV2::Connection);
    if(m_msg_index < (m_testcase.size() - 1)) {
        assert(connection_hdr == Marvin::Http::HeadersV2::ConnectionKeepAlive);
        REQUIRE(connection_hdr == Marvin::Http::HeadersV2::ConnectionKeepAlive);
    } else {
        assert(connection_hdr == Marvin::Http::HeadersV2::ConnectionClose);
        REQUIRE(connection_hdr == Marvin::Http::HeadersV2::ConnectionClose);
    }
    /**
    ...
    */
//    std::string ch_uuid = rdr->getHeader(Marvin::Http::HeadersV2::ConnectionHandlerId);
//    std::string rh_uuid = rdr->getHeader(Marvin::Http::HeadersV2::RequestHandlerId);
    if( m_msg_index == 0 ) {
        m_ch_uuid = ch_uuid;
        m_rh_uuid = rh_uuid;
    } else {
...
void PipelineTest::exec()
{
    ...
    if( m_msg_index < (m_testcase.size() - 1))
        m_msg->setHeader(Marvin::Http::HeadersV2::Connection, Marvin::Http::HeadersV2::ConnectionKeepAlive);
    else
        m_msg->setHeader(Marvin::Http::HeadersV2::Connection, Marvin::Http::HeadersV2::ConnectionClose);
