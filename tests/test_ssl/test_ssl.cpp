...
/// \brief makes a GET request to the url provided and expects status=200/302 and a non empty content
    MessageBaseSPtr  makeRequest(std::string request_uri) {
        ...
        msg->header(Marvin::HeadersV2::Connection, Marvin::HeadersV2::ConnectionClose);
        msg->header(Marvin::HeadersV2::AcceptEncoding, "identity");
        msg->header(Marvin::HeadersV2::TE, "");
