...
void fillMsgRdrAsResponse_01(MessageReaderSPtr msgRdr)
{
    ...
    msgRdr->setHeader(Marvin::Http::HeadersV2::Connection, Marvin::Http::HeadersV2::ConnectionClose);
...
void verifyResponse_01(MessageBaseSPtr msg)
{
    ...
    REQUIRE(msg->getHeader(Marvin::Http::HeadersV2::Connection) == Marvin::Http::HeadersV2::ConnectionClose);
    ...
    REQUIRE(msg->getHeader(Marvin::Http::HeadersV2::Date) == trim(" Sun, 24 Nov 2013 01:38:41 GMT"));
    /// ETag just not passed down
    REQUIRE( ! msg->hasHeader("Etag"));
    REQUIRE( ! msg->hasHeader(Marvin::Http::HeadersV2::ETag));
    /// proxy transforms chunked encoding to content-length style
    REQUIRE( ! msg->hasHeader("Transfer-Encoding"));
    REQUIRE( ! msg->hasHeader(Marvin::Http::HeadersV2::TransferEncoding));
    REQUIRE(msg->hasHeader("Content-Length"));
    REQUIRE(msg->hasHeader(Marvin::Http::HeadersV2::ContentLength));
...
//    msgRdr->setHeader(Marvin::Http::HeadersV2::Host, "example.org");
    ...
    msgRdr->setHeader(Marvin::Http::HeadersV2::AcceptEncoding,"deflate, gzip, x-gzip, identity, *;q=0");
    msgRdr->setHeader(Marvin::Http::HeadersV2::Connection,"Keep-Alive, TE");
    msgRdr->setHeader("TE","deflate, gzip, chunked, trailer");
    msgRdr->setHeader(Marvin::Http::HeadersV2::TransferEncoding,"chunked");
    msgRdr->setHeader(Marvin::Http::HeadersV2::ETag,"1928273tefadseercnbdh");
    std::string s = "012345678956";
    Marvin::BufferChainSPtr bdy = Marvin::BufferChain::makeSPtr(s);
...
void verifyRequest_01(MessageBaseSPtr msgSPtr)
{
    ...
    /// host name has port
    REQUIRE(msgSPtr->getHeader(Marvin::Http::HeadersV2::Host) == "example.org:9999" );
    REQUIRE(msgSPtr->getHeader(Marvin::Http::HeadersV2::AcceptEncoding) == "identity");
    REQUIRE(msgSPtr->getHeader(Marvin::Http::HeadersV2::Connection) == Marvin::Http::HeadersV2::ConnectionClose);
    REQUIRE(msgSPtr->getHeader(Marvin::Http::HeadersV2::TE) == "");
    ...
    REQUIRE( ! msgSPtr->hasHeader(Marvin::Http::HeadersV2::TransferEncoding));
    REQUIRE( ! msgSPtr->hasHeader(Marvin::Http::HeadersV2::ETag));
}
...
//    helpers::fillRequestFromUri(*msgRdr, "http://username:password@somewhere.com/subdirpath/index.php?a=1111#fragment");
    msgRdr->setHeader(Marvin::Http::HeadersV2::Connection, Marvin::Http::HeadersV2::ConnectionKeepAlive);
    std::string s = "012345678956";
    Marvin::BufferChainSPtr bdy = Marvin::BufferChain::makeSPtr(s);
    msgRdr->setContent(bdy);
}
...
bool verifyRequest_MimimumRequirements(MessageBaseSPtr msgSPtr)
{
    ...
    CHECK_FALSE(uri == "");
    CHECK( msgSPtr->hasHeader(Marvin::Http::HeadersV2::Host));
    CHECK( msgSPtr->hasHeader(Marvin::Http::HeadersV2::Connection));
    {
    auto bb = ( (msgSPtr->hasHeader(Marvin::Http::HeadersV2::ContentLength)) || (msgSPtr->hasHeader(Marvin::Http::HeadersV2::TransferEncoding)));
    CHECK(bb);
    }
    if(msgSPtr->hasHeader(Marvin::Http::HeadersV2::ContentLength) && (msgSPtr->getHeader(Marvin::Http::HeadersV2::ContentLength) != "0" )){
        int cl = std::stoi(msgSPtr->getHeader(Marvin::Http::HeadersV2::ContentLength));
